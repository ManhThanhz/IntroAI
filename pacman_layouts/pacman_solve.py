class State:
    def __init__(self, pacman_position, food_points, parent_state=None, parent_action=None):
        self.pacman_position = pacman_position
        self.food_points = food_points
        self.parent_state = parent_state
        self.parent_action = parent_action
        self.cost = 0  # Initialize cost attribute
...
class UCS:
    ...
    def get_successors(self, state):
        ...
        for dx, dy, action in actions:
            ...
            if self.is_valid_position(new_x, new_y):
                next_state = self.get_next_state(state, (new_x, new_y))
    ...
    def get_next_state(self, state, next_position):
...
def visualize(maze, actions):
    ...
    for action in actions:
        os.system('cls' if os.name == 'nt' else 'clear')  # Clear the console
        print_maze(maze, action)
        time.sleep(0.5)  # Pause for 0.5 seconds between actions
...
if __name__ == "__main__":
    layout_file = "D:\\UniBachelor\\232\\AI\\BTL1\\pacman_layouts\\pacman_layouts\\smallMaze.lay"
    algorithm = "UCS"  # or "UCS"
